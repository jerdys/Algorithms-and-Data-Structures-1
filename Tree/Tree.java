    ...
    public void add(T parent, T children) {
        if (parent == null) {
            if (root == null) {
                ...
                System.out.println("Oups..root already added");
            }
        }
        else {
            if (root == null) {
            ...
            if (root.getData().equals(parent)) {
                ...
                if (findParent(root.getChildrens(), parent, children)) {
                    System.out.println("Added successfully!");
                }
                else {
                    System.out.println("Oups..parent not found");
                }
            }
        }
    }
    ...
    private boolean findParent(List<Node<T>> list, T parent, T children) {
        for (int i = 0; i < list.size(); i++) {
            ...
            if (node.getChildrens().size() != 0) {
                if (findParent(node.getChildrens(), parent, children)) {
                    return true;
                }
            }
